...
IPERF_RPM = os.path.join(PACKAGES_DIR, "iperf_64-2.0.8-1.aix5.1.ppc.rpm")
IPERF_TEST_SCRIPT = os.path.join(SCRIPTS_DIR, "test_network_throughput.sh")
...
def test_deploy():
    ...
    Body: {host, username, password, port, label, mode (shutdown|restart),
           seconds, target_path, ping_targets (optional list of IPs)}
    """
    data = request.get_json(force=True) or {}
    host      = (data.get("host") or "").strip()
    ...
    ping_targets = data.get("ping_targets") or []  # list of IP strings
    try:
        port = int(data.get("port") or 22)
    except (TypeError, ValueError):
        port = 22
    ...
    # ── Step 1b: deploy ping targets config (if provided) ──────
    if isinstance(ping_targets, list) and ping_targets:
        # Build the vios_res_net_targets.conf content
        conf_lines = ["# vios_res_client network targets configuration",
                      "# Format: interface=target_ip",
                      "# Generated by PowerPilot"]
        for idx, ip in enumerate(ping_targets):
            ip = str(ip).strip()
            if ip:
                conf_lines.append(f"en{idx}={ip}")
        conf_content = "\n".join(conf_lines) + "\n"

        # Write the config file via SSH command (printf to avoid echo issues)
        remote_conf = f"{target_dir}/vios_res_net_targets.conf"
        # Escape single quotes in content for shell safety
        safe_content = conf_content.replace("'", "'\\''")
        write_cmd = f"printf '%s' '{safe_content}' > '{remote_conf}'"
        wres = ssh_manager.run_command(
            host=host, port=port, username=username, password=password,
            command=write_cmd, timeout=30,
        )
        conf_ok = wres.get("ok", False) or (wres.get("exit_code", 1) == 0)
        conf_msg = f"Deployed {len(ping_targets)} ping target(s) to {remote_conf}"
        if not conf_ok:
            conf_msg = wres.get("error", "Failed to write ping targets config")
        steps.append({"step": 2, "label": "Deploy ping targets config",
                      "ok": conf_ok, "output": conf_msg,
                      "error": "" if conf_ok else conf_msg})
...
def test_results():
    ...
    for entry in sorted(os.listdir(TESTRESULT_DIR)):
        ...
        if entry.endswith(".tar.gz") or entry.endswith(".log"):
...
def test_report():
    """Generate the consolidated VIOS resilience Word report.
    Accepts optional JSON body {"files": ["vios_res_*.tar.gz", ...]} to limit
    which archives are included.  When omitted, all vios_res_* archives are used."""
...
@app.route("/api/test/iperf-report", methods=["POST"])
def test_iperf_report():
    """Generate a Network Throughput Word report from iperf logs in testresult/.
    Accepts optional JSON body {"files": ["file1.log", ...]} to limit which
    logs are included.  When omitted, all iperf logs are used."""
    report_script = os.path.join(SCRIPTS_DIR, "generate_iperf_report.py")
    if not os.path.isfile(report_script):
        return jsonify({"ok": False, "error": "generate_iperf_report.py not found"}), 404
    os.makedirs(TESTRESULT_DIR, exist_ok=True)
    ...
    report_name = "Network_Throughput_Report.docx"
    return jsonify({
        "ok": True,
        "message": "iperf report generated.",
        "report": report_name,
        "output": proc.stdout,
    })


@app.route("/api/test/consolidated-report", methods=["POST"])
def test_consolidated_report():
    """Generate a single consolidated Word report merging all selected test
    result files (iperf logs + VIOS archives) into one document.
    Accepts JSON body {"files": ["file1", "file2", ...]}."""
    report_script = os.path.join(SCRIPTS_DIR, "generate_consolidated_report.py")
    if not os.path.isfile(report_script):
        return jsonify({"ok": False, "error": "generate_consolidated_report.py not found"}), 404
    os.makedirs(TESTRESULT_DIR, exist_ok=True)
    ...
    report_name = "Consolidated_Test_Report.docx"
    return jsonify({
        "ok": True,
...
@app.route("/api/test/iperf", methods=["POST"])
def test_iperf():
    """Run a full iperf network throughput test between two LPARs.

    Body: {server: {host, username, password, port},
           client: {host, username, password, port},
           iperf_port, duration, udp_bandwidth}
    """
    data = request.get_json(force=True) or {}
    srv = data.get("server") or {}
    cli = data.get("client") or {}

    srv_host = (srv.get("host") or "").strip()
    srv_user = (srv.get("username") or "root").strip()
    srv_pass = srv.get("password") or ""
    try:
        srv_port = int(srv.get("port") or 22)
    except (TypeError, ValueError):
        srv_port = 22

    cli_host = (cli.get("host") or "").strip()
    cli_user = (cli.get("username") or "root").strip()
    cli_pass = cli.get("password") or ""
    try:
        cli_port = int(cli.get("port") or 22)
    except (TypeError, ValueError):
        cli_port = 22

    try:
        iperf_port = int(data.get("iperf_port") or 5201)
    except (TypeError, ValueError):
        iperf_port = 5201
    try:
        ...
        duration = 10
    udp_bw = (data.get("udp_bandwidth") or "1G").strip() or "1G"

    if not srv_host or not cli_host:
        return jsonify({"ok": False, "error": "Both server and client host are required"}), 400
    if not os.path.isfile(IPERF_TEST_SCRIPT):
        return jsonify({"ok": False, "error": "test_network_throughput.sh not found"}), 404

    steps = []
    step_num = 0

    def _ensure_iperf(label, host, port, username, password):
        nonlocal step_num
        step_num += 1
        check = ssh_manager.run_command(
            host=host, port=port, username=username, password=password,
            command="command -v iperf && iperf --version 2>&1 | head -1",
            timeout=30)
        if check.get("ok") and check.get("output", "").strip():
            steps.append({"step": step_num, "label": f"Check iperf on {label}",
                          "ok": True, "output": check["output"].strip()})
            return True
        if not os.path.isfile(IPERF_RPM):
            steps.append({"step": step_num, "label": f"Check iperf on {label}",
                          "ok": False,
                          "error": "iperf not found and RPM not in packages/"})
            return False
        step_num += 1
        rpm_name = os.path.basename(IPERF_RPM)
        dep = ssh_manager.deploy_file(
            local_path=IPERF_RPM, host=host, port=port,
            username=username, password=password,
            target_path=f"/tmp/{rpm_name}", method="sftp")
        steps.append({"step": step_num, "label": f"Deploy iperf RPM to {label}",
                      "ok": dep.get("ok", False),
                      "output": dep.get("message", ""),
                      "error": dep.get("error", "")})
        if not dep.get("ok"):
            return False
        step_num += 1
        inst = ssh_manager.run_command(
            host=host, port=port, username=username, password=password,
            command=f"rpm -i /tmp/{rpm_name} 2>&1 || echo 'note: may already be installed'",
            timeout=60)
        steps.append({"step": step_num, "label": f"Install iperf on {label}",
                      "ok": True, "output": (inst.get("output") or "").strip()})
        step_num += 1
        v = ssh_manager.run_command(
            host=host, port=port, username=username, password=password,
            command="command -v iperf && iperf --version 2>&1 | head -1",
            timeout=30)
        ok = bool(v.get("ok") and v.get("output", "").strip())
        steps.append({"step": step_num, "label": f"Verify iperf on {label}",
                      "ok": ok,
                      "output": v.get("output", "").strip() if ok else "",
                      "error": "" if ok else "iperf still not found after install"})
        return ok

    # ── Ensure iperf on both LPARs ────────────────────────────
    if not _ensure_iperf("Server", srv_host, srv_port, srv_user, srv_pass):
        return jsonify({"ok": False, "error": "Failed to ensure iperf on server", "steps": steps})
    if not _ensure_iperf("Client", cli_host, cli_port, cli_user, cli_pass):
        return jsonify({"ok": False, "error": "Failed to ensure iperf on client", "steps": steps})

    # ── Deploy test script to both LPARs ──────────────────────
    remote_script = "/tmp/test_network_throughput.sh"
    for lbl, h, p, u, pw in [
        ("Server", srv_host, srv_port, srv_user, srv_pass),
        ("Client", cli_host, cli_port, cli_user, cli_pass),
    ]:
        step_num += 1
        dep = ssh_manager.deploy_file(
            local_path=IPERF_TEST_SCRIPT, host=h, port=p,
            username=u, password=pw,
            target_path=remote_script, method="sftp")
        steps.append({"step": step_num, "label": f"Deploy test script to {lbl}",
                      "ok": dep.get("ok", False),
                      "output": dep.get("message", ""),
                      "error": dep.get("error", "")})
        if not dep.get("ok"):
            return jsonify({"ok": False,
                            "error": f"Deploy to {lbl} failed: {dep.get('error')}",
                            "steps": steps})

    # ── Kill stale iperf & start server ────────────────────────
    step_num += 1
    ssh_manager.run_command(
        host=srv_host, port=srv_port, username=srv_user, password=srv_pass,
        command=f"kill $(lsof -ti :{iperf_port}) 2>/dev/null; sleep 1; true",
        timeout=15)
    start_cmd = (f"nohup iperf -s -p {iperf_port} "
                 f">/tmp/iperf_server.log 2>&1 & echo $!")
    started = ssh_manager.run_command(
        host=srv_host, port=srv_port, username=srv_user, password=srv_pass,
        command=start_cmd, timeout=15)
    srv_pid = (started.get("output") or "").strip()
    steps.append({"step": step_num, "label": "Start iperf server",
                  "ok": started.get("ok", False),
                  "output": f"PID: {srv_pid}" if srv_pid else "",
                  "error": started.get("error", "")})
    if not started.get("ok"):
        return jsonify({"ok": False, "error": "Failed to start iperf server",
                        "steps": steps})
    ...
    # ── Run client test ────────────────────────────────────────
    step_num += 1
    client_cmd = (f"cd /tmp && chmod +x test_network_throughput.sh && "
                  f"./test_network_throughput.sh client {srv_host} "
                  f"{iperf_port} {duration} {udp_bw}")
    result = ssh_manager.run_command(
        host=cli_host, port=cli_port, username=cli_user, password=cli_pass,
        command=client_cmd, timeout=max(duration * 3 + 60, 120))
    steps.append({"step": step_num, "label": "Run iperf client (TCP + UDP)",
                  "ok": result.get("ok", False),
                  "output": (result.get("output") or "")[-2000:],
                  "error": result.get("error", "")})

    # ── Download result log from client ────────────────────────
    step_num += 1
    find_cmd = "ls -1t /tmp/*_network_throughput/*_result.log 2>/dev/null | head -1"
    found = ssh_manager.run_command(
        host=cli_host, port=cli_port, username=cli_user, password=cli_pass,
        command=find_cmd, timeout=30)
    remote_log = (found.get("output") or "").strip().splitlines()
    remote_log = remote_log[0].strip() if remote_log else ""
    if remote_log:
        os.makedirs(TESTRESULT_DIR, exist_ok=True)
        local_name = f"iperf_{os.path.basename(remote_log)}"
        local_path = os.path.join(TESTRESULT_DIR, local_name)
        dl = ssh_manager.download_file(
            host=cli_host, port=cli_port, username=cli_user, password=cli_pass,
            remote_path=remote_log, local_path=local_path)
        steps.append({"step": step_num, "label": "Download client result log",
                      "ok": dl.get("ok", False),
                      "output": dl.get("message", ""),
                      "error": dl.get("error", "")})
    else:
        steps.append({"step": step_num, "label": "Download client result log",
                      "ok": False, "error": "No result log found on client"})

    # ── Download server log ────────────────────────────────────
    step_num += 1
    srv_find = ("ls -1t /tmp/*_network_throughput/*_result.log "
                "/tmp/iperf_server.log 2>/dev/null | head -1")
    srv_found = ssh_manager.run_command(
        host=srv_host, port=srv_port, username=srv_user, password=srv_pass,
        command=srv_find, timeout=30)
    srv_remote = (srv_found.get("output") or "").strip().splitlines()
    srv_remote = srv_remote[0].strip() if srv_remote else ""
    if srv_remote:
        os.makedirs(TESTRESULT_DIR, exist_ok=True)
        srv_dl = ssh_manager.download_file(
            host=srv_host, port=srv_port, username=srv_user, password=srv_pass,
            remote_path=srv_remote,
            local_path=os.path.join(TESTRESULT_DIR,
                                    f"iperf_srv_{os.path.basename(srv_remote)}"))
        steps.append({"step": step_num, "label": "Download server log",
                      "ok": srv_dl.get("ok", False),
                      "output": srv_dl.get("message", ""),
                      "error": srv_dl.get("error", "")})
    else:
        steps.append({"step": step_num, "label": "Download server log",
                      "ok": True, "output": "No separate server log (OK)"})

    # ── Cleanup — kill iperf server ────────────────────────────
    step_num += 1
    if srv_pid:
        ssh_manager.run_command(
            host=srv_host, port=srv_port, username=srv_user, password=srv_pass,
            command=f"kill {srv_pid} 2>/dev/null; sleep 1; true", timeout=15)
    steps.append({"step": step_num, "label": "Cleanup (kill iperf server)",
                  "ok": True,
                  "output": f"Killed PID {srv_pid}" if srv_pid else "Done"})

    all_ok = all(s.get("ok") for s in steps)
    return jsonify({
        "ok": all_ok,
        "message": ("iperf test completed." if all_ok
                    else "iperf test completed with some issues."),
        "steps": steps,
    })
